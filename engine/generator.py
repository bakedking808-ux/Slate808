...
NATURAL_TRAVEL_INTENT_PATTERNS = (
    ...
    rf"^(?:quiet|calm|short|little)\s+break\s+near\s+\w+\b.*\b(?:{MONTHS}|budget|for\s+\d+)\b",
    r"\bthinking\s+of\s+heading\s+somewhere\s+coastal\b",
    r"\b(?:could\s+you\s+help\s+me\s+)?plan\s+something\s+(?:around\s+the\s+lakeside|affordable\s+for\s+a\s+few\s+days)\b",
    r"\bthinking\s+of\s+a\s+place\s+with\s+wildlife\b",
