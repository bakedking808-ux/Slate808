...
def test_get_log_file_path_returns_session_log_path(monkeypatch, tmp_path):
...
def test_create_session_log_dir_increments_sequentially(monkeypatch, tmp_path):
    monkeypatch.setattr(logger, "LOGS_ROOT", tmp_path / "logs")
    monkeypatch.setattr(logger, "local_date_str", lambda: "2026-04-19")

    session_one = logger.create_session_log_dir()
    session_two = logger.create_session_log_dir()
    session_three = logger.create_session_log_dir()

    assert session_one.name == "session-001"
    assert session_two.name == "session-002"
    assert session_three.name == "session-003"


def test_log_run_writes_engine_log_inside_session_directory(monkeypatch, tmp_path):
    monkeypatch.setattr(logger, "LOGS_ROOT", tmp_path / "logs")
    monkeypatch.setattr(logger, "local_date_str", lambda: "2026-04-19")
...
def test_log_event_writes_named_log_inside_session_directory(monkeypatch, tmp_path):
    monkeypatch.setattr(logger, "LOGS_ROOT", tmp_path / "logs")
    monkeypatch.setattr(logger, "local_date_str", lambda: "2026-04-19")
...
def test_append_log_writes_plain_line_inside_session_directory(monkeypatch, tmp_path):
    monkeypatch.setattr(logger, "LOGS_ROOT", tmp_path / "logs")
    monkeypatch.setattr(logger, "local_date_str", lambda: "2026-04-19")
